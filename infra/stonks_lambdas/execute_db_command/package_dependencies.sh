...
aws lambda publish-version --function-name stonks-lambdas-ExecuteDBCommand > /dev/null
aws lambda update-function-configuration --function-name stonks-lambdas-ExecuteDBCommand --layers $latest_layer_arn --region us-east-1  >> /dev/null

# aws lambda get-function-configuration --function-name stonks-lambdas-ExecuteDBCommand --region us-east-1  >> /dev/null
